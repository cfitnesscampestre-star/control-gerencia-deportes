...
function pCard(aid,p){
  ...
  return `<button class="pcard" data-act="openProfesor" data-id="${esc(p.id)}" style="--ac:${areaColor(aid)}">
    ...
    <div class="pc-r">${pill(p.tipo||'Planta','ok')}${p.activo===false?pill('Inactivo','mut'):''}<small>${esServ(aid)?plu(svRegsRecientes(aid,p.id),'consulta en 30 días','consultas en 30 días'):plu(n,'clase','clases')}</small></div>
  </button>`;
}
...
function openProfesor(pid){
  ...
  openModal(`${mHead(pid?(ro?pfNom(aid,0):'Editar '+pfNom(aid,0).toLowerCase()):'Nuevo '+pfNom(aid,0).toLowerCase())}
    ...
    ${pid?`<div class="btns"><button class="btn danger" data-act="delProfesor" data-id="${esc(pid)}">Eliminar ${esc(pfNom(aid,0).toLowerCase())}</button></div>`:''}`}`);
}
