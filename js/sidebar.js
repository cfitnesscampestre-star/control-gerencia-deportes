...
function viewSidebar(){
  ...
  const nav = ger
    ...
    : prof ? NAV_PROF.map(n=>`<button class="${ui.pTab===n.id?'on':''}" data-act="pTab" data-tab="${n.id}">${ic(n.ic)}<span>${n.label}</span></button>`).join('')
    : (esGim(session.area)?NAV_GIM:NAV_DIR).map(n=>`<button class="${ui.aTab===n.id?'on':''}" data-act="aTab" data-tab="${n.id}">${ic(n.ic)}<span>${n.label}</span></button>`).join('');
  ...
  return `<aside class="sidebar">
    <div class="sb-brand">
      ...
      <small>${ger?'Gerencia de deportes':prof?'Portal del profesor':'Dirección de área'}</small></div>
    </div>
    <nav class="sb-nav">${nav}</nav>
    ${areas}
    <div class="sb-foot">
      <div class="sb-user"><b>${ger?'Gerencia':prof?'Profesor':'Dirección'}</b>${ger?'Todas las áreas':prof?esc(pr.nombre):areaIco(a,{size:15})+' '+esc(a.nombre)}</div>
