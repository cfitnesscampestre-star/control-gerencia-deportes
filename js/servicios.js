...
const SV_ESTADO = {atendido:'Se atendió', no_asistio:'No se presentó', cancelo:'Canceló'};
...
function svHorarioTxt(p){
  ...
  const runs=[]; for(let i=0;i<7;i++){ const h=H[i]; if(!h) continue; const k=h.i+'-'+h.f, l=runs[runs.length-1];
    if(l&&l.k===k&&l.b===i-1) l.b=i; else runs.push({a:i,b:i,k,i:svMin(h.i),f:svMin(h.f)}); }
  return runs.map(r=>`${DIAS[r.a]}${r.b>r.a?'–'+DIAS[r.b]:''} ${svRangoTxt(r.i,r.f)}`).join(' · ');
}
...
function svDiaProf(aid,p,f,regs){
  const att=regs.filter(svAtendido), per=regs.filter(x=>!svAtendido(x));
  const uA=svUnion(att.map(svIv).filter(Boolean)), uP=svUnion(per.map(svIv).filter(Boolean));
  const r={att,per,turno:0,enTurno:0,fuera:0,idle:0,perdida:0,libre:0,win:null,blocks:[]};
  if(!regs.length||f>todayStr()||!svHor(p)) return r;
  const win0=svTurnoDia(p,f);
  if(!win0){ r.fuera=svLen(uA); return r; }              // vino en un día sin turno
  const win=win0.slice();
  if(f===todayStr()){ const n=new Date(), now=n.getHours()*60+n.getMinutes(); if(now<=win[0]) return r; win[1]=Math.min(win[1],now); }
  const dentro=svClip(uA,win[0],win[1]), huecos=svHuecos(win,uA);
  r.win=win; r.turno=win[1]-win[0]; r.enTurno=svLen(dentro); r.fuera=svLen(uA)-svLen(svClip(uA,win0[0],win0[1])); r.idle=svLen(huecos);
  const perd=[]; huecos.forEach(h=>svClip(uP,h[0],h[1]).forEach(x=>perd.push(x)));
  r.perdida=svLen(perd); r.libre=r.idle-r.perdida;
  r.blocks=[...dentro.map(x=>({a:x[0],b:x[1],t:'att'})),...perd.map(x=>({a:x[0],b:x[1],t:'perd'}))];
  return r;
}
...
function servStats(aid,desde,hasta,pid,B){
  const t=todayStr(), fin=hasta>t?t:hasta; B=B||bitacora(aid);
  const todas=B.filter(x=>x.f>=desde&&x.f<=fin&&(!pid||x.profId===pid)), regs=todas.filter(svAtendido), per=todas.filter(x=>!svAtendido(x));
...
function svDiaInfo(aid,f,B){
  ...
  let est='off';
  if(f>t) est='fut';
  else if(s.todas.length) est='hay';
  else if(f<t&&profesores(aid).some(p=>p.activo!==false&&spDias(p).includes(w)&&svPrimera(p.id,B)&&svPrimera(p.id,B)<=f)) est='sin';
  return {f,est,n:s.n,min:s.min,util:s.util,noLlego:s.noLlego};
}
function svCalendario(aid){
  const mes=ui.sv.ref.slice(0,7), [y,m]=mes.split('-').map(Number), nd=new Date(y,m,0).getDate(), off=wdIdx(`${mes}-01`), t=todayStr(), B=bitacora(aid), cel=[];
  for(let i=0;i<off;i++) cel.push('<i class="sv-cal-x"></i>');
  for(let d=1; d<=nd; d++){
    const f=`${mes}-${String(d).padStart(2,'0')}`, di=svDiaInfo(aid,f,B);
    cel.push(`<button class="sv-cal-d ${di.est}${di.est==='hay'&&di.util!=null?' u-'+svUtilCls(di.util):''}${f===t?' hoy':''}" data-act="svDia" data-f="${f}" aria-label="${esc(fmtLarga(f))}: ${plu(di.n,'servicio','servicios')}"><span class="n">${d}</span><b>${di.n||(di.est==='hay'?'0':'')}</b>${di.est==='hay'?`<small>${di.util!=null?di.util+'%':svHmC(di.min)}</small>`:''}</button>`);
  }
  return `<div class="sv-cal">
    ...
    <div class="sv-leg"><span><i class="u-ok"></i>ocupación ≥ ${SV_UTIL.ok}% (justo)</span><span><i class="u-warn"></i>${SV_UTIL.warn}–${SV_UTIL.ok-1}% (revisar)</span><span><i class="u-bad"></i>&lt; ${SV_UTIL.warn}% (tiempo muerto excesivo)</span><span><i class="sin"></i>día de turno sin bitácora</span></div>
    <div class="sub">El número grande son los servicios del día y abajo la ocupación del turno. Toca un día para ver lo que dio cada ${esc(servDe(aid).esp.toLowerCase())}.</div>
  </div>`;
}
function vServInicio(aid){
  ...
  return `<div class="sv">
    ...
    ${sinH.length?`<div class="sv-warn">${ic('bolt')}<span>Falta el horario de ${esc(sinH.map(p=>p.nombre).join(', '))}: sin él no se puede medir el tiempo muerto.</span></div>`:''}
    ${svPeriodoBar()}
    ${svKpis(aid,s)}
    <div class="h2">Por ${esc(S.esp.toLowerCase())} <small class="mut">${esc(R.txt)}</small></div>
    ${svTarjetasEsp(aid,R)}
    <div class="h2">Calendario <small class="mut">toca un día para ver el detalle</small></div>
    ${svCalendario(aid)}
    <div class="btns no-print"><button class="btn" data-act="svPrintReporte">Imprimir reporte: ${esc(R.txt.toLowerCase())}</button></div>
  </div>`;
}

/* ---------- un renglón de la bitácora ---------- */
function svLineaReg(aid,r,i,edit){
  const ok=svAtendido(r), iv=svIv(r), rt=svRetraso(r), tag=edit?'button':'div';
  const hora=iv?svRangoTxt(iv[0],iv[1]):svHm(r.min), tarde=(rt!=null&&rt>60)?`<span class="sv-late">capturado ${esc(svHm(rt))} después</span>`:'';
  return `<${tag} class="line sv-l${ok?'':' sv-no'}"${edit?` data-act="svReg" data-id="${esc(r.id)}" style="--ac:${areaColor(aid)}"`:''}><div class="t">${i+1}</div><div class="b"><b>${esc(r.paciente)}</b>
    <small>${ok?'':`<span class="sv-tag">${esc(SV_ESTADO[r.estado]||'No llegó')}</span> `}${esc(hora)} · ${esc(r.servicio||'')}${ok?' · '+esc(svOrigenTxt(r.origen)):''}</small>
...
function svDiaModal(f){
  ...
  const cards=filas.map(x=>{
    const s=servStats(aid,f,f,x.id,B), d=svDiaProf(aid,x.p,f,x.regs), tw=svTurnoDia(x.p,f);
    return `<div class="sv-card"><div class="sv-esp-h"><b>${esc(x.p?x.p.nombre:'Especialista dado de baja')}</b><span class="sv-hm">${esc(svHm(s.min))}</span></div>
      <div class="sv-hor-t">${tw?`Turno ${esc(svRangoTxt(tw[0],tw[1]))}`:(x.p&&spDias(x.p).length?'Ese día no tenía turno':'Sin horario')}</div>
      <div class="sv-esp-t">${svFrase(aid,s)}</div>
...
function vServProf(){
  ...
  const tw=svTurnoDia(p,f), dp=svDiaProf(aid,p,f,L);
  return `<div class="sv">
    <div class="sub">${hoy?'Registra cada servicio con su hora de inicio y de fin. Tu agenda la llevas por separado.':'Estás viendo otro día.'}</div>
    <div class="af-date">
    ...
    <div class="sv-hor-t">${tw?`Tu turno: <b>${esc(svRangoTxt(tw[0],tw[1]))}</b>`:(spDias(p).length?'Este día no tienes turno.':'Todavía no tienes horario capturado.')}</div>
    ...
    <div class="btns"><button class="btn primary block" data-act="svReg">+ Registrar servicio</button></div>
    ${dp.win?`<div class="sv-esp-t">${svTurnoHTML(d)}</div>${svLinea(dp)}`:''}
    <div class="h2">Mi bitácora del día</div>
    ${L.length?L.map((r,i)=>svLineaReg(aid,r,i,true)).join(''):empty('Todavía no registras servicios este día.')}
    ${svAviso(aid)}
  </div>`;
}
...
function openReg(id){
  ...
  const est=r.estado||'atendido', nombres=[...new Set(mis.map(x=>x.paciente))].sort((a,b)=>a.localeCompare(b,'es')).slice(0,400);
  ...
  openModal(`${mHead(id?'Editar registro':'Nuevo registro')}
    <div class="sub">${esc(fmtLarga(fecha))} · <b>Servicio ${num}</b></div>
    <label class="f"><span>Nombre del socio</span><input id="rg_nom" list="rg_dl" value="${esc(r.paciente)}" autocomplete="off" autocapitalize="words" placeholder="Nombre y apellido"><datalist id="rg_dl">${nombres.map(n=>`<option value="${esc(n)}">`).join('')}</datalist></label>
    <label class="f"><span>¿Qué pasó con la cita?</span><select id="rg_est">${Object.keys(SV_ESTADO).map(k=>`<option value="${k}"${k===est?' selected':''}>${SV_ESTADO[k]}</option>`).join('')}</select></label>
    <label class="f"><span>Servicio</span><select id="rg_srv">${S.servicios.map(x=>`<option value="${esc(x.s)}"${x.s===srv?' selected':''}>${esc(x.s)}</option>`).join('')}</select></label>
    <div class="two">
      <label class="f"><span id="rg_hi_l">Hora de inicio</span><input id="rg_hi" type="time" step="300" value="${esc(hi)}"></label>
    ...
    <label class="f" id="rg_ori_w"><span>¿Lo canalizó alguna área?</span><select id="rg_ori">${svOrigenOpts(aid,r.origen||'propia')}</select></label>
    ${svAviso(aid)}
  ...
  svRegRefresh();
  setTimeout(()=>{ const i=$('#rg_nom'); if(i&&!id) i.focus(); },60);
}
...
Object.assign(actions,{
  ...
  svReg(d){ if(roDatos(session.area)) return; openReg(d.id||''); },
  ...
  saveReg(d){
    ...
    const id=d.id||('b'+uid());
    setPath(`data/${aid}/bitacora/${id}`,{id,f:fecha,profId:pid,paciente:nom,servicio:$('#rg_srv').value,estado:est,hi:svHHMM(hi),hf:svHHMM(hf),min:hf-hi,origen:est==='atendido'?$('#rg_ori').value:'propia',ts:prev.ts||Date.now()});
  ...
  svCopiarHorario(){                                     // copia el horario del primer día marcado a los demás días marcados
    const filas=[...document.querySelectorAll('.sv-hr')].filter(r=>r.querySelector('.pf_dia').checked); if(filas.length<2) return;
    const i=filas[0].querySelector('.pf_hi').value, f=filas[0].querySelector('.pf_hf').value;
    filas.slice(1).forEach(r=>{ r.querySelector('.pf_hi').value=i; r.querySelector('.pf_hf').value=f; });
...
document.addEventListener('change',e=>{ if(e.target.classList&&e.target.classList.contains('pf_dia')){ const r=e.target.closest('.sv-hr'); if(r) r.classList.toggle('on',e.target.checked); } });
...
function svProfCampos(aid,p,ro){
  ...
  return `<div class="f"><span class="lb">Horario de trabajo</span>
    <div class="sv-hor">${DIAS.map((l,i)=>{ const h=H[i]; return `<div class="sv-hr${h?' on':''}"><label class="sv-d"><input type="checkbox" class="pf_dia" value="${i}"${h?' checked':''}${dis}><span>${l}</span></label>
      <input type="time" class="pf_hi" step="300" value="${esc(h?h.i:'09:00')}"${dis}><em>a</em><input type="time" class="pf_hf" step="300" value="${esc(h?h.f:'17:00')}"${dis}></div>`; }).join('')}</div>
    ${ro?'':'<button type="button" class="btn sm" data-act="svCopiarHorario">Copiar el primer día marcado a los demás marcados</button>'}
    <small class="mut">Marca los días que trabaja y anota su hora de entrada y de salida. Con esto se compara lo que registra contra su turno para calcular el tiempo muerto.</small></div>`;
}
function svProfLeer(aid){
  if(!esServ(aid)) return {};
  const horario={};
  document.querySelectorAll('.sv-hr').forEach(r=>{ const c=r.querySelector('.pf_dia'); if(c.checked) horario[c.value]={i:r.querySelector('.pf_hi').value,f:r.querySelector('.pf_hf').value}; });
  return {horario};
}
function svProfError(aid){
  ...
  for(const r of filas){ const a=svMin(r.querySelector('.pf_hi').value), b=svMin(r.querySelector('.pf_hf').value), d=DIAS[+r.querySelector('.pf_dia').value];
...
function svInsights(aid,s,sp,B){
  ...
  if(!s.n&&!s.noLlego) return [{c:'info',h:'Sin registros',x:`No hay ${S.unidadP} ni otros servicios registrados en el período.`}];
...
function pServicios(d){
  ...
  if(!ids.length) return empty('Nutrición y Fisioterapia no están dentro del filtro de área. Elige “Todas las áreas” o el área que quieras.');
...
function svKpisResumen(){                               // dos tarjetas más en el Resumen de gerencia
  const ids=servAreaIds(); if(!ids.length) return '';
...
function simServicio(a,ai,D,profs,rnd,t,nombreAlumno){
  const fis=a.tipo==='fisioterapia', S=SERV[a.tipo], rn=(x,y)=>x+Math.floor(rnd()*(y-x+1));
  D.bitacora={};
  const espec=fis?['Rehabilitación deportiva','Terapia manual','Ortopédica']:['Nutrición deportiva','Control de peso'];
  ...
  const pesos=fis?[.10,.42,.16,.14,.06,.08,.04]:[.55,.28,.08,.04,.05];
  const elige=()=>{ let r=rnd(), i=0; while(i<pesos.length-1&&r>pesos[i]){ r-=pesos[i]; i++; } return S.servicios[i]; };
  const pacientes=Array.from({length:fis?70:55},(_,i)=>nombreAlumno(ai*61+i*13+7));
